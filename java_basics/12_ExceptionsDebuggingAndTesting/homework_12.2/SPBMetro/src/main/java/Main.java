...
public class Main {
    ...
    private static final Marker STATIONS = MarkerManager.getMarker("Stations");
    private static final Marker ERRSTATIONS = MarkerManager.getMarker("ErrStations");
    ...
    private static Station takeStation(String message) {
        for (; ; ) {
            ...
            if (station != null) {
                logger.info(STATIONS, "Станция найдена: " + line);
                return station;
            }
            if (line.isEmpty()) throw new IllegalArgumentException("Вы не ввели данные");

            logger.info(ERRSTATIONS, "Станция не найдена: " + line);
